# Awesome Public Data Sources
[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)  
Curated list of high-quality **public data providers**, open data portals, government APIs, dataset repositories, and other accessible sources to inspire data projects, analysis, and exploration.

This is a living collection of reliable places to find datasets, from general platforms like Kaggle to specialized ones like CDC or data.gov. Contributions welcome!

## Contents
- [General / Aggregators](#general--aggregators)
- [Government & Public Sector](#government--public-sector)
- [Health & Medicine](#health--medicine)
- [Finance & Economics](#finance--economics)
- [Science & Research](#science--research)
- [Geospatial & Environment](#geospatial--environment)
- [Social Sciences & Demographics](#social-sciences--demographics)
- [Machine Learning & AI-Focused](#machine-learning--ai-focused)
- [APIs & Real-Time Data](#apis--real-time-data)
- [Other / Niche](#other--niche)
- [Contributing](#contributing)
- [License](#license)

## General / Aggregators
- [Kaggle](https://www.kaggle.com/datasets) — Massive community-driven dataset hub with competitions and notebooks.
- [Google Dataset Search](https://datasetsearch.research.google.com/) — Search engine across thousands of open data repositories.
- [Google BigQuery Datasets](https://cloud.google.com/datasets) — a wide variety of public datasets through its Public Dataset Program.
- [data.world](https://data.world/) — Collaborative platform for finding, sharing, and querying datasets.
- [UCI Machine Learning Repository](https://archive.ics.uci.edu/) — Classic ML datasets.
- [Huwise Hub](https://hub.huwise.com/pages/home/) — Public open data portal hosting thousands of free, curated datasets (government, health, geospatial, etc.) with visualization tools, APIs, and self-service access.
- [World Bank Open Data](https://data.worldbank.org/) — Free and open access to global development data.

## Government & Public Sector
- [data.gov](https://www.data.gov/) — The home of the U.S. Government's open data (300k+ datasets).
- [USAFacts](https://usafacts.org/) — Non-partisan data on U.S. economy, population, health, etc.
- [USAspending.gov](https://www.usaspending.gov/) — U.S. government's primary portal for federal spending transparency, covering contracts, grants, loans, and awards with searchable/downloadable data and APIs.
- [European Data Portal](https://data.europa.eu/) — Aggregates open data from EU countries.

## Health & Medicine
- [CDC Data & Statistics](https://www.cdc.gov/datastatistics/index.html) — Public health datasets from the Centers for Disease Control.
- [HHS Open Data](https://cdo.hhs.gov/s/open-data) — U.S. Department of Health and Human Services open data portal.
- [WHO Global Health Observatory](https://www.who.int/data/gho) — Global health statistics.

## Finance & Economics
- Submit a pull request to add a source.

## Science & Research
- Submit a pull request to add a source.

## Geospatial & Environment
- Submit a pull request to add a source.

## Social Sciences & Demographics
- Submit a pull request to add a source.

## Machine Learning & AI-Focused
- Submit a pull request to add a source.

## APIs & Real-Time Data
- Submit a pull request to add a source.

## Other / Niche
- Submit a pull request to add a source.

## Contributing
Contributions are welcome! Submit a pull request.

To add a source:
- It should be publicly accessible (free or freemium).
- Prefer high-quality, reliable providers.
- Format: `- [Provider Name](URL) — Brief one-sentence description.`

## License
CC0 1.0 Universal (public domain)
